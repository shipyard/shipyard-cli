...
func TestEmbeddedLoopRequiresCoverage(t *testing.T) {
	...
	required := map[string]string{
		...
		"otherwise call\n  `rebuild_environment` once":        "a refused restart falls back to one rebuild, only when nothing is processing",
		"If the result starts with `Cannot restart`":          "the fallback keys on the tool's real refusal text",
...
func TestEmbeddedLoopPlansBeforeChecking(t *testing.T) {
	...
	required := map[string]string{
		...
		"--bypass-token)":                                                                            "commands fetch the token instead of spelling it out",
		"expands `$SHIPYARD_TOKEN` in a command's\nown arguments before a prefix assignment applies": "curl needs && so the cookie is not empty",
		"say in the report that the token appeared in commands":                                      "the literal-token fallback is disclosed",
	}

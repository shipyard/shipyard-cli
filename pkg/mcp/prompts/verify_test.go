...
func TestEmbeddedLoopRequiresCoverage(t *testing.T) {
	...
	required := map[string]string{
		...
		"otherwise call `rebuild_environment` once":           "a refused restart falls back to one rebuild, only when nothing is processing",
		"If its result starts with `Cannot restart`":          "the fallback keys on the tool's real refusal text",
		"`retired` → call `revive_environment`":               "a retired environment is revived, not restarted",

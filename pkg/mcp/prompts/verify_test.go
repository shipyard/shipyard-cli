...
func TestEmbeddedLoopRequiresCoverage(t *testing.T) {
	...
	required := map[string]string{
		...
		"call\n  `rebuild_environment` once instead": "a restart refused as not paused falls back to one rebuild",
		"**Any other environment** (the base branch's, another pull request's): never start it": "only the change's own environment is ever started",
